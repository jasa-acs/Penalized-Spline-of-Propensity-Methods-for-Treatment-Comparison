...
### this produce Figure 3 in the paper
...
pdf(paste(DIREC, "FiguresandTables/Figure3_relativeRMSE_Plots.pdf", sep=""))
...
text(2.75, max(temp)-0.10, paste("IPTW=",round(iptw1y[3],2)), col="green4")
...
arrows(2, max(temp)-0.02, 2, max(temp), length=0.1)
text(1.85, max(temp)-0.10, paste("G-COMP=",round(gcompute2y[2],2)), col="black")
...
axis(side = 4, at = seq(0.2, 1.2, 0.2), tck = -0.03)
abline(h=1)
arrows(3, max(temp)-0.02, 3, max(temp), length=0.1)
text(2.75, max(temp)-0.12, paste("IPTW=",round(iptw3y[3],2)), col="green4")
arrows(2, max(temp)-0.02, 2, max(temp), length=0.1)
text(1.85, max(temp)-0.10, paste("G-COMP=",round(gcompute3y[2],2)), col="black")
...
plot(pspp1x, pspp2y,pch=17, ylim=c(min(temp), max(temp)),col="red", yaxt='n', xaxt='n',
     main="", cex=2.5)
points(aipw1x, aipw2y, pch=8, col="blue", cex=2.5)
points(gcompute1x,gcompute2y, pch=1, col="black", cex=2.5)
points(iptw1x,iptw2y, pch=22, col="green4", cex=2.5)
