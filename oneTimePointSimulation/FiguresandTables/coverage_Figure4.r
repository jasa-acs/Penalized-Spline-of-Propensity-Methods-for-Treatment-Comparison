...
###PENCOMP_Result: folder when PENCOMP results are 
#IPTW_Result: folder when IPTW results are 
#gcompute_Result: folder when g computation results are 
#AIPTWResult: folder when AIPTW results are 
### this produce Figure 4 in the paper

####directory where results and plots are stored
DIREC="codes/oneTimePointSimulation/FiguresandTables/" ###where files are stored after combineResult_step2.R

pdf(paste(DIREC, "FiguresandTables/Figure4_coverage_Plots.pdf", sep=""))


par(mfrow = c(2,3),
    oma = c(2,2,3,2) + 0.1,
    mar = c(3,0.5,0,0) + 0.1, cex.main = 1.5)
...
mtext(expression(bold(paste(Delta, "  linear"))), line = 0, at=2, side=3)
...
arrows(2, max(temp)-0.5, 2, max(temp), length=0.1)
text(1.85, max(temp)-1, paste("G-COMP=",round(gcompute2y[2],2)), col="black")
...
text(1.23, max(temp)-1, paste("IPTW=",round(iptw3y[1],2)), col="green4")

arrows(2, max(temp)-0.5, 2, max(temp), length=0.1)
text(1.9, max(temp)-3, paste("G-COMP=",round(gcompute3y[2],2)), col="black")
text(1.9, max(temp)-2, paste("AIPW=",round(aipw3y[2],2)), col="blue")
text(1.9, max(temp)-1, paste("IPTW=",round(iptw3y[2],2)), col="green4")
...
axis(side = 2, at =seq(0, 10, 2), tck = -0.03, labels = seq(0, 10, 2))
abline(h=5)
#arrows(2, max(temp)-1, 2, max(temp), length=0.1)
#text(1.75, max(temp)-3, paste("G-COMPutation=",round(gcompute1y[2],2)))
axis(side = 1, at = c(1, 2, 3), labels = c("A", "B", "C"), tck = -0.01)
mtext(expression(bold(paste(Delta, "  nonlinear"))), line = 0, at=2, side=3)
...
text(1.85, max(temp)-1, paste("G-COMP=",round(gcompute3y[2],2)), col="black")
