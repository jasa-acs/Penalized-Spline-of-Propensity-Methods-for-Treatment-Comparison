...
for (k in start:end) {
  ...
  #################################naive estimate ###################################### 
  naiveEst=NULL
  naiveEst=naive(dataInput=simdatFit, original=1)
  
  ########bootstrap samples
  mulResult=NULL
  mulResult=replicate(numT, naive(dataInput=simdatFit, original=0) )
  
  
  for(ind in c(0, 1, 2) ){
    tableNaive[k, (1:4)+(4*ind)] = c( naiveEst[ind+1], sd(mulResult[ind+1,], na.rm = T), naiveEst[ind+1] + c(-1,1)*1.96*sd(mulResult[ind+1, ], na.rm = T) )
  }

  ############store in the result folders###########################
  
  write.table(tableNaive, paste0(DIREC, "Results/tableNaive.txt"), sep="\t", row.names = F, 
              quote = F)
