...
for (k in start:end) {
  ...
  ### AIPTW estimates
  ###########################without weight truncation #############################################
  ###
  aiptwEst=NULL
  aiptwEst=AIPTW(dataInput=simdatFit, baselineVar=baselineVar, original=1, numCarlo=numCarlo, truncation=NULL, visit = k)  
  
  ########bootstrap samples
  mulResult=NULL
  mulResult=replicate(numT, AIPTW(dataInput=simdatFit, baselineVar=baselineVar, original=0, numCarlo=numCarlo, truncation=NULL, visit=k)  )
  
  for(ind in c(0, 1:2) ){
    tableAIPTW[k, (1:4)+(4*ind)] = c( aiptwEst[ind+1], sd(mulResult[ind+1,], na.rm = T), aiptwEst[ind+1] - 1.96*sd(mulResult[ind+1, ], na.rm = T),
                                      aiptwEst[ind+1] + 1.96*sd(mulResult[ind+1, ], na.rm = T) )
  }
  
  
  ###########################with weight truncation at 1% #########################################################
  ###
  aiptwEst=NULL
  aiptwEst=AIPTW(dataInput=simdatFit, baselineVar=baselineVar, original=1, numCarlo=numCarlo, truncation=0.01, visit=k)  
  
  ########bootstrap samples
  mulResult=NULL
  mulResult=replicate(numT, AIPTW(dataInput=simdatFit, baselineVar=baselineVar, original=0, numCarlo=numCarlo, truncation=0.01, visit = k)  )
  
  for(ind in c(0, 1:2) ){
    tableAIPTW_truncate[k, (1:4)+(4*ind)] = c( aiptwEst[ind+1], sd(mulResult[ind+1,], na.rm = T), aiptwEst[ind+1] - 1.96*sd(mulResult[ind+1, ], na.rm = T),
                                               aiptwEst[ind+1] + 1.96*sd(mulResult[ind+1, ], na.rm = T) )
  }
  
  
  write.table(tableAIPTW, paste0(DIREC, "Results/tableAIPTW.txt"), sep="\t", row.names = F, 
              quote = F)
  
  write.table(tableAIPTW_truncate, paste0(DIREC, "Results/tableAIPW_truncate.txt"), sep="\t", row.names = F, 
              quote = F)
