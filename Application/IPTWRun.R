...
for (k in start:end) {
  ...
  ################## IPTW estimates
  iptwEst=NULL
  iptwEst=IPTW(dataInput=simdatFit, original=1, truncate=0.01)
  
  ########bootstrap samples
  mulResult=NULL
  mulResult=replicate(numT, IPTW(dataInput=simdatFit, original=0, truncate=0.01) )
  
  
  for(ind in c(0, 1:5) ){
    tableIPTW[k, (1:4)+(4*ind)] = c( iptwEst[ind+1], sd(mulResult[ind+1,], na.rm = T), iptwEst[ind+1] + c(-1,1)*1.96*sd(mulResult[ind+1, ], na.rm = T) )
  }
  

  
  write.table(tableIPTW[, 1:12], paste0(DIREC, "Results/tableIPTW.txt"), sep="\t", row.names = F, 
              quote = F)
  
  write.table(tableIPTW[, 13:24], paste0(DIREC, "Results/tableIPTW_truncate.txt"), sep="\t", row.names = F, 
              quote = F)
